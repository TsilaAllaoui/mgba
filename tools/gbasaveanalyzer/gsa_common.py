...
def default_interactive_mgba_config_home() -> Path:
    """Persistent XDG root for interactive mgba mGBA sessions.
    ...
    """
    if os.environ.get("mgba_MGBA_CONFIG_HOME"):
        return Path(os.environ["mgba_MGBA_CONFIG_HOME"]).expanduser().resolve()
    s=load_settings().get("interactive_mgba_config_home")
    if s:return Path(s).expanduser().resolve()
    return (Path.home()/".config"/"mgba"/"mgba-user").resolve()

def settings_path() -> Path:
    override=os.environ.get("mgba_SETTINGS")
    if override:
        return Path(override).expanduser().resolve()
    return Path.home()/".config"/"mgba"/"settings.json"
...
def find_executable(names, script_file:Path, env_names=()):
    """Find a mgba companion executable without requiring shell setup.
    ...
    subs=(
        ...
        'build-mgba-qt', 'build-mgba-qt/qt', 'build-mgba-qt/sdl',
        'build-r64-qt', 'build-r64-qt/qt', 'build-r64-qt/sdl',
        'build-r64-headless', 'build-mgba-headless',
        'build-r6-headless', 'build-r3-static', 'build-gsa',
...
def find_manifest(patched:Path, explicit:str|None=None)->Path:
    if explicit:
        ...
        return p
    for p in [Path(str(patched)+'.gsa.json'),patched.with_suffix('.gsa.json'),patched.with_suffix('.gbabr.json')]:
