...
from gsa_common import (default_library_folder, default_rom_folder, default_virtual_cart_folder,
...
def detect_tools():
    ...
    autoqa=find_executable(['mgba-mgba-autoqa','mgba-gbabr-autoqa'],Path(__file__),['mgba_AUTOQA','MGBA_GBABR_AUTOQA'])
    selftest=find_executable(['mgba-mgba-selftest','mgba-gbabr-selftest'],Path(__file__),['mgba_SELFTEST','MGBA_GBABR_SELFTEST'])
    gui=find_executable(['mgba-Qt','mgba-qt'],Path(__file__),['mgba_GUI'])
...
def pick_game(lib:Path, prompt='Select game'):
    ...
    while True:
        ...
        # Let gsa_record do its normal exact/fuzzy resolution for text queries.
        return v
...
------------------------------------------------------------
 ...
 10. Build / rebuild Qt6 GUI in WSL
...
''')
        ...
        if choice=='1':
            ...
            run('gsa_scan.py',default_roms,'--jobs',jobs,'--frames',frames,'--library',default_lib,*extra)
        elif choice=='2':
            latest_files(lib)
        elif choice in ('3','4'):
            ...
            run('gsa_record.py',*args)
        elif choice=='5':
            jobs=ask('Parallel jobs',str(min(8,os.cpu_count() or 4)))
            extra=[]
            if patcher: extra += ['--patcher',str(patcher)]
            if autoqa: extra += ['--autoqa',str(autoqa)]
            run('gsa_regress.py','--library',default_lib,'--jobs',jobs,*extra)
        elif choice=='6':
            saves=ask('Folder containing .sav files')
            if saves: run('gsa_import_saves.py',saves,'--library',default_lib)
        elif choice=='7':
            rom=ask('ROM file')
            ...
            if patcher:extra += ['--patcher',str(patcher)]
            if autoqa:extra += ['--autoqa',str(autoqa)]
            run('gsa_patch_test.py',rom,target,'--frames',frames,'--fresh',*extra)
        elif choice=='8':
            ...
            subprocess.call([str(ROOT/'BUILD_QT_WSL.sh')])
        elif choice=='11':
            ...
            run('gsa_bundle.py','--library',default_lib)
        elif choice=='14':
            run('gsa_virtual_cart.py','--cart-dir',default_carts)
        elif choice=='15':
            run('gsa_virtual_cart.py','--cart-dir',default_carts,'--list')
        elif choice=='16':
            subprocess.call([str(HERE/'test.sh')])
