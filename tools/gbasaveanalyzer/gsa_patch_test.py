...
from gsa_common import create_manifest,find_executable
...
def main()->int:
    ...
    report=out.with_suffix('.patch.txt'); manifest=out.with_suffix('.gsa.json')
    print('[1/3] Patching...')
    ...
    test=Path(__file__).with_name('gsa_test.py')
