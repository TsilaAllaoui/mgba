...
from mgba_common import (default_interactive_mgba_config_home, default_virtual_cart_folder,
...
def _frontend(explicit:str|None, prefer_qt:bool=False)->Path:
    ...
    names=(['mgba-qt','mgba-sdl'] if prefer_qt else
           ['mgba-sdl','mgba-qt'])
    p=find_executable(names,Path(__file__),['MGBA_FRONTEND'])
    if not p:
        raise FileNotFoundError('No mGBA SDL/Qt executable found. Rebuild with ./build.sh --clean or pass --gui /full/path/to/mgba-sdl')
    return Path(p)
...
def main()->int:
    ap=argparse.ArgumentParser(description='Boot an existing melonDS/GBAVirtualCart backing directly in mGBA.')
    ...
    ap.add_argument('--mgba-config-home',help='persistent mGBA interactive config root')
