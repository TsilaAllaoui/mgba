...
def choose_target(root:Path,g:dict,explicit:str|None)->tuple[str,Path,Path,int]:
    ...
    levels=previous_levels(root,g['sha256'])
    for t in candidates:
        td=target_dir(root,g['sha256'],t);patched=td/'patched.gba';man=td/'manifest.mgba.json'
...
def main()->int:
    ...
    gui=Path(args.gui).resolve() if args.gui else find_executable(['mgba-qt','mgba-sdl'],Path(__file__),['MGBA_QT'])
    if not gui:
    ...
    print('\nONE-TIME GUI RECORDING — mGBA v1.0')
